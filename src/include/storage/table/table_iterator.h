...
class TableIterator {
 ...
 public:
  ...
  inline bool operator==(const TableIterator &itr) const {
    LOG_INFO("left %d %d right %d %d",tuple_->rid_.GetPageId(),tuple_->rid_.GetSlotNum(),itr.tuple_->rid_.GetPageId(),tuple_->rid_.GetSlotNum());
  ...
  TableIterator &operator=(const TableIterator &other) {
    ...
    LOG_INFO("tuple rid %ld",tuple_->rid_.Get());
    LOG_INFO("=2");
    txn_ = other.txn_;
    LOG_INFO("=END");
    return *this;
